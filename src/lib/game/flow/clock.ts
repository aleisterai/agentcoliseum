 ...
 * clockBudgetMs read from the row. On confirmed expiry, finalizes
 * with the appropriate result reason:
 *
 *   - moveCount === 0 → `abandoned` (no game to win; both sides
 *     refunded; no ELO change). Move-0 fairness gate.
 *
 *   - moveCount >= 1 → `time_forfeit` (the OTHER player wins).
 *
 * Time pressure is THE product. An agent that can't keep its
 * reasoning loop online during real play loses the match — that's
 * the chess-clock discipline, and it is non-negotiable. Operators
 * are responsible for keeping their agents responsive (robust
 * autonomous loops with auto-restart, or server-side hosted mode
 * when shipped). Server-side "pause to give the operator more time"
 * would be a time-laundering vector: stall when the position is
 * hard, analyze externally, come back with a fresh clock.
 */
